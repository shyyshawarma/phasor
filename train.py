...
#     # Save Model Checkpoint
#     # ===============================
#     checkpoint_path = os.path.join(checkpoint_dir, f"netG_model_epoch_{epoch}_psnr_{avg_psnr:.4f}.pth")
#     torch.save({
#         'epoch': epoch,
...
if opt.resume:
    ...
    net_g.load_state_dict(checkpoint['model_state_dict'])
    optimizer_g.load_state_dict(checkpoint['optimizer_state_dict'])
    net_g_scheduler.load_state_dict(checkpoint['scheduler_state_dict'])
    start_epoch = checkpoint['epoch'] + 1
    print(f"===> Resumed from epoch {checkpoint['epoch']}")
else:
...
checkpoint_dir = '/kaggle/working/phasor/ckpts'
os.makedirs(checkpoint_dir, exist_ok=True)
...
for epoch in range(start_epoch, opt.niter + opt.niter_decay + 1):
    ...
    for iteration, batch in enumerate(data_loader_train, 1):
        ...
        if iteration % 100 == 0:
            out_image = torch.cat((rgb, fake_b, tarL), 3)
            save_img(out_image[0].detach().cpu(), f'{output_dir_train}/{iteration}.png')
            print(f"===> Epoch[{epoch}]({iteration}/{len(data_loader_train)}): Loss_G: {loss_g.item():.6f}")
    ...
    torch.save({
        ...
        'model_state_dict': net_g.state_dict(),
        'optimizer_state_dict': optimizer_g.state_dict(),
        'scheduler_state_dict': net_g_scheduler.state_dict(),
        'avg_psnr': avg_psnr,
    }, checkpoint_path)
    print(f"Checkpoint saved at {checkpoint_path}")
